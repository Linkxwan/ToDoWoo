        ...
        if request.POST['password1'] == request.POST['password2']:
            try:
                ...
                return render(request, 'todo/signup_user.html',
                              {'form': UserCreationForm(), 'error': 'That username is already teken'})
        else:
            return render(request, 'todo/signup_user.html',
...
def login_user(request):
    if request.method == 'GET':
        ...
        if user is None:
            return render(request, 'todo/login_user.html', {'form': AuthenticationForm(), 'error': 'Username and pasword did not match'})
        else:
            login(request, user)
            return redirect('current_todos')
